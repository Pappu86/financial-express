...
import { CategoryBangla } from '~/types/category'
...
export default defineComponent({
  ...
  setup() {
    ...
    const lang = i18n?.locale === 'bangla' ? 'en' : 'en' // For now 'en' will be selected language for bangla category all time.

    const fetch = async () => {
      loading.value = true
      try {
        const category: CategoryBangla = await $axios.$get<CategoryBangla>(
          `api/${lang}/home/category-bangla`
        )
...
<template>
  <div id="home-category-bangla" ref="target" class="bg-bg2-light dark:bg-bg2-dark pb-8 pt-4">
    ...
    <v-container>
      ...
      <div class="grid xl:grid-cols-4 gap-4 pt-4 mb-4">
        <article v-for="post in column3" :key="post.id">
          ...
          <div class="bg-op-light dark:bg-op-dark p-2 h-[9.3rem] overflow-hidden">
            ...
            >
              <nuxt-link :to="localePath(post.slug)" class="line-clamp-3 py-1.5">{{ post.title }}</nuxt-link>
            </v-heading>
            <p
              class="text-p-light dark:text-p-dark text-base leading-6 line-clamp-3"
              v-html="post.excerpt"
            ></p>
          </div>
        </article>
