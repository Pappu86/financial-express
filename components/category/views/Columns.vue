...
export default defineComponent({
  ...
  setup() {
    ...
    const nextPageUrl = ref(`api/en/category/views/columns`)
      ...
      try {
        const response: CategoryViewsChild =
          await $axios.$get<CategoryViewsChild>(
            `api/${lang}/category/views/columns`
          )

        nextPageUrl.value = `api/en/category/views/sub-more/columns/${response.id}`
        const items: Posts[] = [response.featured, ...response.posts]
        posts1.value = items.slice(0, 2)
        posts2.value = items.slice(2, 8)
      } finally {
        loading.value = false
      }
    }
...
<template>
  <div id="columns" ref="target" class="py-4">
    <v-container>
      <div class="grid grid-cols-1 xl:grid-cols-16 gap-4 pb-8">
        ...
        >
          <div class="col-span-full">
            <div class="grid grid-cols-1 xl:grid-cols-12 xl:grid-rows-3 gap-4">
              <article
                v-for="post in posts1"
                :key="post.id"
                class="col-auto xl:col-span-3 xl:row-span-3 flex flex-col border-t-2 border-bg7-light dark:border-bg7-dark"
              >
              ...
              <article
                ...
                :key="post.id"
                class="col-auto xl:col-span-3 xl:row-span-1 flex flex-col border-t-2 border-bg7-light dark:border-bg7-dark"
              >
                <div class="bg-bg2-light dark:bg-bg2-dark p-2 flex-1">
                  <v-heading
                    class="font-semibold text-h3-light dark:text-h3-dark text-base xl:text-xl transition-colors hover:text-rose-600 dark:hover:text-rose-600"
          ...
          <div class="col-span-full w-full h-0.5 bg-bg5-light dark:bg-bg5-dark my-4" />
          <div class="col-span-full">
