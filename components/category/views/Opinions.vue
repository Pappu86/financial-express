            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 xl:grid-rows-2 gap-4">
              ...
              >
                <div class="bg-bg2-light dark:bg-bg2-dark p-2 flex-1">
                  ...
                  >
                    <nuxt-link :to="localePath(post.slug)" class="line-clamp-4">{{ post.title }}</nuxt-link>
                  </v-heading>
                  <div v-if="post.reporter">
                    <p class="text-h2-light dark:text-h2-dark text-xs xl:text-xl">
                      <nuxt-link :to="'/'">{{ post.reporter.name }}</nuxt-link>
                    </p>

                    <figure v-if="post.image" class="float-right mt-4">
                      <v-img :src="post.image" :alt="post.caption" class="rounded-full w-20 h-20" />
                    </figure>
                  </div>
                </div>
              </article>
